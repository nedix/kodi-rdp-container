...
RUN for PREVIOUS_FEDORA_VERSION in $(seq $(( FEDORA_VERSION - 2 )) "$FEDORA_VERSION"); do \
        sed -E \
        ...
        && sed -E \
            ...
            > "/etc/yum.repos.d/rpmfusion-free-${PREVIOUS_FEDORA_VERSION}.repo" \
...
RUN dnf install -y gdb strace top ps
